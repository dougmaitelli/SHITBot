...
describe("event reminders", () => {
  it("posts due reminders without allowing mentions and removes them", async () => {
    ...
    await store.setEvent({
      id: "event", guildId: "guild", channelId: "event-channel", messageId: "event-message",
      creatorId: "creator", name: "Picnic", startsAt: now + 3600, rsvps: {}, createdAt: Date.now(),
    });
    await store.setReminder({
      id: "reminder", guildId: "guild", channelId: "reminders", creatorId: "creator",
      targetRef: "event:event", sendAt: now, note: "Bring lunch @everyone", createdAt: Date.now(),
    });
    ...
    const client = {
      channels: {
        fetch: async () => ({
          isTextBased: () => true,
          isSendable: () => true,
          send: async (message: { content: string; allowedMentions: { parse: string[] } }) => { sent.push(message); },
        }),
      },
