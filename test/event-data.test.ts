...
describe("assistant event data", () => {
  it("combines and orders upcoming events and movie nights", async () => {
    ...
    await store.set({
      id: "night", guildId: "guild", channelId: "movies", messageId: "message", creatorId: "creator",
      startsAt: 300, location: "Home", movie: null, votingOpen: true, rsvps: { user: "yes" },
      suggestions: [], createdAt: 0,
    });
    await store.setEvent({
      id: "event", guildId: "guild", channelId: "events", messageId: "message", creatorId: "creator",
      name: "Picnic", startsAt: 200, attendanceLimit: 3, rsvps: { user: "yes", maybe: "maybe" }, createdAt: 0,
    });

    const items = upcomingItems(store, "guild", 100);
    assert.deepEqual(items.map((item) => item.ref), ["event:event", "movie-night:night"]);
