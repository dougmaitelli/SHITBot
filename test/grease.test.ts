...
describe("greaseSucceeds", () => {
  it("succeeds for rolls below five percent", () => {
    ...
    assert.equal(greaseSucceeds(() => 0), true);
    assert.equal(greaseSucceeds(() => 0.049999), true);
  });

  it("fails for rolls at or above five percent", () => {
    assert.equal(greaseSucceeds(() => 0.05), false);
    assert.equal(greaseSucceeds(() => 0.99), false);
  });
});
