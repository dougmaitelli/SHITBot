...
interface DiscordItem { item: UpcomingItem; scheduled: GuildScheduledEvent }
...
function creationArguments(value: unknown): EventToolArguments {
  ...
  if (input.duration_minutes !== undefined && (!Number.isInteger(input.duration_minutes) || (input.duration_minutes as number) < 15 || (input.duration_minutes as number) > 10080)) throw new Error("Duration must be from 15 to 10080 minutes.");
  if (input.attendance_limit !== undefined && (!Number.isInteger(input.attendance_limit) || (input.attendance_limit as number) < 1 || (input.attendance_limit as number) > 100000)) throw new Error("Attendance limit must be from 1 to 100000.");
  ...
  return [...scheduled.values()]
    ...
    .map((event) => {
      ...
      const item: UpcomingItem = local
        ...
        : {
            ref: `discord-event:${event.id}`, kind: "event", guildId: guild.id,
            channelId: event.channelId ?? "", messageId: "", creatorId: event.creatorId ?? "",
            title: event.name, startsAt: Math.floor(event.scheduledStartTimestamp! / 1000),
            details: [event.description, event.entityMetadata?.location].filter(Boolean).join("\n") || undefined,
            rsvps: {}, url: event.url, discordInterestedCount: event.userCount ?? undefined,
          };
...
export function registerEventAssistantTools(
  client: Client, store: BotStore, tools: AssistantTool[], timeZone: string, movieNightsChannel: string,
): void {
  ...
  tools.push({
    ...
    parameters: { type: "object", additionalProperties: false, required: ["name", "when"], properties: {
    ...
    async execute(context, value) {
      ...
      try { link = parseEventLink(input.link); } catch { throw new Error("The event link must be a valid http or https URL."); }
      const channel = await client.channels.fetch(context.channelId);
  ...
  tools.push({
    ...
    description: "List all upcoming Discord Scheduled Events in this server, including events created manually or by other bots.",
  ...
  tools.push({
    ...
    description: "List upcoming Discord Scheduled Events the requesting user marked Interested in, plus bot events where they RSVP'd Going. Can include bot Maybe responses.",
    ...
    async execute(context, value) {
      ...
      for (const { item, scheduled } of await visible(context)) {
        if (accepted.has(item.rsvps[context.userId] ?? "") || await isInterested(scheduled, context.userId)) matches.push(item);
        if (matches.length >= requestedLimit(input.limit)) break;
      }
      return JSON.stringify({ events: matches.map((item) => itemSummary(item, context.userId)), total_returned: matches.length });
    },
  });

  tools.push({
    name: "get_event_attendance",
    description: "Get attendance for one Discord Scheduled Event. Bot-created events include RSVP breakdowns; other events include Discord's Interested count.",
    parameters: { type: "object", additionalProperties: false, required: ["event_id"], properties: { event_id: { type: "string", description: "Stable ID returned by list_upcoming_events" } } },
    async execute(context, value) {
      const input = objectArguments(value);
  ...
  tools.push({
    ...
    description: "Post or schedule an organizer-only reminder for an upcoming Discord Scheduled Event. Omit when to post now.",
