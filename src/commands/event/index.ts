...
const createEventCommand: CommandFactory = ({ client, store, config }): CommandModule => {
  ...
  async function create(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const event: CommunityEvent = {
      id: randomUUID().slice(0, 8), guildId: interaction.guildId, channelId: interaction.channelId,
      messageId: "", creatorId: interaction.user.id, name,
      startsAt, description: interaction.options.getString("description")?.trim() || undefined,
      link, attendanceLimit: interaction.options.getInteger("attendance-limit") ?? undefined,
      rsvps: {}, createdAt: Date.now(),
    };
    await interaction.deferReply();
    try {
      event.scheduledEventId = await createScheduledEvent(interaction.guild, event, interaction.options.getInteger("duration") ?? 180);
    } catch (error) {
      console.error("Could not create Discord scheduled event", error);
      await interaction.deleteReply().catch(() => undefined);
      await interaction.followUp({ content: "I couldn't create the Discord event. Check that I have the **Create Events** permission.", flags: MessageFlags.Ephemeral });
      return;
    }
    try {
      const message = await interaction.editReply(renderEvent(event));
