...
import { deleteScheduledEvent, updateScheduledEvent } from "./scheduled-event.js";
...
const createEventCommand: CommandFactory = ({ client, store, config, assistantTools }): CommandModule => {
  ...
  async function edit(interaction: GuildCommandInteraction): Promise<void> {
  ...
  return {
    ...
    async execute(interaction) {
      ...
      else if (subcommand === "edit") await edit(interaction);
    },
    handleInteraction,
    onReady() {
