...
import { parseScheduledEventReference } from "../event/create-event.js";
...
import { deleteScheduledEvent, updateScheduledEvent, updateScheduledEventMovie } from "./scheduled-event.js";
...
const createMovieNightCommand: CommandFactory = ({ client, store, config, assistantTools }): CommandModule => {
  ...
  async function editNight(interaction: GuildCommandInteraction): Promise<void> {
    ...
    try {
      ...
      await updateNightMessages(updated);
  ...
  return {
    ...
    async execute(interaction: GuildCommandInteraction): Promise<void> {
      ...
      if (subcommand === "create") await createNight(interaction);
      else if (subcommand === "edit") await editNight(interaction);
    },
    handleInteraction,
    onReady(): void {
