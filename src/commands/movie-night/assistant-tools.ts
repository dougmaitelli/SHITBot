...
function creationArguments(value: unknown): MovieNightToolArguments {
  ...
  if (input.duration_minutes !== undefined && (!Number.isInteger(input.duration_minutes) || (input.duration_minutes as number) < 30 || (input.duration_minutes as number) > 720)) throw new Error("Duration must be from 30 to 720 minutes.");
...
export function registerMovieNightAssistantTools(
  ...
  tools.push({
    ...
    async execute(context, value) {
      const channel = await requireMovieChannel(context.channelId) as Awaited<ReturnType<Client["channels"]["fetch"]>>;
  ...
  tools.push({
    ...
    async execute(context, value) {
      ...
      const detailed = await Promise.all(matches.map(async (match) => { try { return await tmdb.getMovieDetails(match.tmdbId); } catch { return match; } }));
      return JSON.stringify({ query, results: detailed.map((movie) => ({
