  ...
  tools.push({
    ...
    parameters: {
      ...
      properties: {
        ...
        movie: {
          type: "string",
          description: "Optional selected movie; omit for suggestions and voting",
        },
        duration_minutes: {
          type: "integer",
          minimum: 30,
  ...
  tools.push({
    ...
    async execute(context, value) {
      ...
      if (channel?.isTextBased() && !channel.isDMBased()) {
        await (await channel.messages.fetch(updated.messageId)).edit(renderNight(updated));
        await Promise.allSettled(
          updated.suggestions
            .filter((suggestion) => suggestion.messageId)
            .map(async (suggestion) =>
              (await channel.messages.fetch(suggestion.messageId!)).edit(renderSuggestion(updated, suggestion)),
            ),
        );
      }
  ...
  tools.push({
    ...
    parameters: {
      ...
      properties: {
        query: {
          type: "string",
          description: "Movie title or concise query, maximum 100 characters",
        },
      },
    },
    async execute(context, value) {
