...
export function upcomingItems(store: BotStore, guildId: string, now = Math.floor(Date.now() / 1000)): UpcomingItem[] {
  ...
  const nights: UpcomingItem[] = store
    .list()
    .filter((night) => night.guildId === guildId && !night.closedAt && night.startsAt > now)
    .map((night) => ({
      ref: `movie-night:${night.id}`,
      kind: "movie-night",
      guildId: night.guildId,
      channelId: night.channelId,
      messageId: night.messageId,
      creatorId: night.creatorId,
      title: night.movie ? `Movie Night: ${night.movie}` : "Movie Night: Movie TBD",
      startsAt: night.startsAt,
      details: `Location: ${night.location}`,
...
export function attendance(item: UpcomingItem) {
  const users = (status: "yes" | "maybe" | "no") =>
    Object.entries(item.rsvps)
  ...
  return {
    ...
    going: going.slice(0, 50),
    maybe: maybe.slice(0, 50),
    notGoing: notGoing.slice(0, 50),
